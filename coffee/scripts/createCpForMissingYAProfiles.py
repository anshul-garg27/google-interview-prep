...
def writeInErrorFile(data_array):
    ...
    file_path = os.path.join(current_dir, "create_missing_profiles_error_YA.csv")
...
def writeInFile(data_array):
    current_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(current_dir, "create_missing_profiles_YA.csv")
    with open(file_path, "a", newline='') as f:
        writer = csv.writer(f)
        writer.writerow(data_array)

def generateCsvForYoutubeHandles():
    youtube_query = 'SELECT channel_id FROM youtube_account order by followers desc nulls last'
    coffee_postgres_cursor.execute(youtube_query)
    result_set = coffee_postgres_cursor.fetchall()
    coffee_postgres_conn.commit()
    try:
        output_file='youtube_handles.csv'
...
current_dir = os.path.dirname(os.path.abspath(__file__))
error_file_path = os.path.join(current_dir, "create_missing_profiles_error_YA.csv")
