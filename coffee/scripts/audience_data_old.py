...
# MAKE ERROR FILE EMPTY ON START
current_dir = os.path.dirname(os.path.abspath(__file__))
file_path = os.path.join(current_dir, "audience_data_error.txt")
...
while True:
    ...
    chunk_results = clickhouse_client.execute(clickhouse_query + ' LIMIT {} OFFSET {}'.format(chunk_size, offset),with_column_types=True)
    offset += chunk_size
    postgres_conn.commit()
    clickhouse_dict_results = [dict(zip([column[0] for column in chunk_results[1]], row)) for row in chunk_results[0]]
   
    if len(clickhouse_dict_results) < 1:
        break
    check_query = 'SELECT * FROM social_profile_audience_info WHERE platform_id = %s'
    errorcount = 0
    for row in clickhouse_dict_results:
        city_array ={}
        ...
        errorcount = InsertUpdate(insert_row,errorcount)
    end_time = time.time()
    exec_time = math.ceil(end_time - start_time)
    print(str(chunk_size* iteration)+ " Completed in "+ str(exec_time) +" seconds and " +str(errorcount) + " has Errors")
