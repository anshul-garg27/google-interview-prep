...
def writeInErrorFile(data_array):
    ...
    file_path = os.path.join(current_dir, "create_missing_profiles_error.csv")
...
def writeInFile(data_array):
    current_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(current_dir, "create_missing_profiles.csv")
    with open(file_path, "a", newline='') as f:
        writer = csv.writer(f)
        writer.writerow(data_array)
...
def generateCsvForYoutubeHandles():
    youtube_query = 'SELECT handle FROM instagram_account order by followers desc nulls last'
...
current_dir = os.path.dirname(os.path.abspath(__file__))
error_file_path = os.path.join(current_dir, "create_missing_profiles_error.csv")
