...
def writeInErrorFile(data_array):
    ...
    file_path = os.path.join(current_dir, "create_missing_profiles_remaining_IA_error.csv")
...
def writeInFile(data_array):
    current_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(current_dir, "create_missing_profiles_remaining_IA.csv")
    with open(file_path, "a", newline='') as f:
        writer = csv.writer(f)
        writer.writerow(data_array)
...
def generateCsvForInstagramHandles():
    instagram_query = '''
                        ...
                        LEFT JOIN campaign_profiles cp ON ia.id = cp.platform_account_id and cp.platform='INSTAGRAM'
                        WHERE cp.id IS null
                    '''
    ...
    try:
        output_file='instagram_remaining_handles.csv'
...
current_dir = os.path.dirname(os.path.abspath(__file__))
error_file_path = os.path.join(current_dir, "create_missing_profiles_remaining_IA_error.csv")
