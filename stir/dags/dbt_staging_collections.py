...
with DAG(
        dag_id="dbt_staging_collections",
    ...
    truncate_fake_ts = ClickHouseOperator(
        ...
        sql=(
            '''
                truncate table dbt.mart_staging_fake_events
            '''
        ),
    ...
    create_ts = DbtRunOperator(
        ...
        select=["tag:staging_collections"],
        target="production",
    ...
    download_cmd = "aws s3 cp s3://gcc-social-data/data-pipeline/tmp/cps_stg.json /tmp/cps_stg_raw.json && sed -e 's/\\\\/\\\\\\\\/g' /tmp/cps_stg_raw.json > /tmp/cps_stg.json"
    ...
    truncate_fake_ts >> create_ts >> fetch_from_ch >> import_to_pg_box >> import_to_pg_db
