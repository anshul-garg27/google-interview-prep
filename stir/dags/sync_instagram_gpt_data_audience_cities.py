...
def create_scrape_request_log():
    ...
    profiles = client.query('''with 
     profile_events as (select handle,
                               argMax(dimensions, event_timestamp) dimension
                        from _e.profile_log_events
                        where source = 'openapi_profile_info_v0.10'
                          and JSONHas(dimensions, 'audience_city')
                          and event_timestamp>='2023-09-05'
                        group by handle),
     semi as (select handle,
                     JSONLength(JSONExtractRaw(dimension, 'audience_city'))         cities
              from profile_events)
    select semi.handle, mia.name, mia.bio
from semi
left join dbt.mart_instagram_account mia on mia.handle = semi.handle
where cities<=4
                                '''
                            f"LIMIT {max_profiles}")

    flow = 'refresh_instagram_gpt_data_audience_cities'
...
with DAG(
        dag_id='sync_instagram_gpt_data_audience_cities',
    ...
    task = PythonOperator(
        task_id='create_instagram_gpt_data_audience_cities_scl',
