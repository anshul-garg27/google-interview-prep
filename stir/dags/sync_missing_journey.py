...
# syncing journey for active ucas where journey not present already


def sync_missing_journey():
    ...
    ucaIds = client.query("""select uca.id
                                    ...
                                    where uca.active = 1
                                    and uca.id  not in (select journey.uca_id from dbt.stg_campaign_user_campaign_journey journey) 
    ...
    for row in ucaIds.result_rows:
        ...
        channel.basic_publish(exchange='campaign.dx',
                              routing_key='campaign.JOURNEY_MIGRATION_FOR_UCA_RK', body=json_message)
    connection.close()


with DAG(
        dag_id='sync_missing_journey',
    ...
    task = PythonOperator(
        task_id='sync_missing_journey',
        python_callable=sync_missing_journey
    )
    task
