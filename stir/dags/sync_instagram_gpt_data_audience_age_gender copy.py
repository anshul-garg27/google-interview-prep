...
def create_scrape_request_log():
    ...
    profiles = client.query('''with abc as (
                                            ...
                                            WHERE (source = 'openapi_profile_info_v0.10'
                                                AND JSONHas(dimensions, 'audience_gender_age')
                                                )
                                            GROUP BY profile_id
                                            ORDER BY count DESC
                                )
                                select name, handle, bio, primary_category
    ...
    flow = 'refresh_instagram_gpt_data_audience_age_gender'
    ...
    for profile in profiles.result_rows:
        data = {"flow": flow, "platform": "INSTAGRAM", "params": {"data": {"handle": profile[1], "name": profile[0], "bio": profile[2], "category": profile[3]}}}
        requests.post(url=url, data=json.dumps(data))


with DAG(
        dag_id='sync_instagram_gpt_data_audience_age_gender',
    ...
    task = PythonOperator(
        task_id='create_instagram_gpt_data__audience_age_gender_scl',
