...
all_collection_profiles as (
    select pci.id id, platform, ia.ig_id profile_id, profile_collection_id
    from {{ref('stg_coffee_profile_collection_item')}} pci
    left join {{ref('stg_coffee_profile_collection')}} pc on pc.id = pci.profile_collection_id
    left join {{ref('stg_coffee_view_instagram_account_lite')}} ia on toString(ia.id) = toString(pci.platform_account_code)
    where platform = 'INSTAGRAM' and toString(pci.platform_account_code) != '' and enabled = True and pc.source = 'SAAS-AT'
),
all_profiles as (
