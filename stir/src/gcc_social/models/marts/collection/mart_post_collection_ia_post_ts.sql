...
collection_posts as (
    ...
    where enabled = True and platform = 'INSTAGRAM'
),
post_short_code as (
...
link_clicks as (
    select concat('https://', url) url, sum(unique_clicks) clicks
    from dbt.mart_link_clicks
    where url in urls
    group by url
),
manual_link_clicks as (
    select mcpc.short_code short_code, post_collection_id, cp.link, max(Clicks) clicks
    from dbt.mart_collection_post_clicks mcpc
    left join collection_posts cp on cp.short_code = mcpc.short_code
    where short_code in post_short_code
    group by short_code, post_collection_id, cp.link
),
link_orders as (
    select
        link url,
...
link_clicks_final as (
    select
    cp.short_code short_code,
    cp.link link,
    if(lc.clicks > ifNull(mlc.clicks, 0), lc.clicks, ifNull(mlc.clicks, 0)) clicks
    from collection_posts cp
    inner join link_clicks lc on lc.url = cp.link
    left join manual_link_clicks mlc on mlc.short_code = cp.short_code
),
post_clicks_orders as (
    select
        ...
        max(lc.clicks) clicks
    from collection_posts cp
    left join link_orders lo on lo.url = cp.link
    left join link_clicks_final lc on lc.link = cp.link
    group by short_code
),
shortcodes as (
    select short_code from collection_posts
...
with_metrics as (
    select
        ...
        CASE
            WHEN pts.likes = 0 THEN coalesce(argMax(pts.likes, stats_date) FILTER(WHERE pts.likes > 0)
            OVER (PARTITION BY short_code ORDER BY stats_date), pts.likes)
            ELSE pts.likes
        END AS likes,
        CASE
            WHEN pts.plays = 0 THEN coalesce(argMax(pts.plays, stats_date) FILTER(WHERE pts.plays > 0)
            OVER (PARTITION BY short_code ORDER BY stats_date), pts.plays)
            ELSE pts.plays
        END AS plays,
        1.0*(CASE
            WHEN pts.real_reach = 0 THEN coalesce(argMax(pts.real_reach, stats_date) FILTER(WHERE pts.real_reach > 0)
            OVER (PARTITION BY short_code ORDER BY stats_date), pts.real_reach)
            ELSE pts.real_reach
        END) AS real_reach,
        1.0*(CASE
            WHEN pts.real_impressions = 0 THEN coalesce(argMax(pts.real_impressions, stats_date) FILTER(WHERE pts.real_impressions > 0)
            OVER (PARTITION BY short_code ORDER BY stats_date), pts.real_impressions)
            ELSE pts.real_impressions
        END) AS real_impressions,
...
all_posts_ts as (
    select
        ...
        max(_fake_reach) fake_reach,
        max(_fake_impressions) fake_impressions,
        
        argMin(real_reach, stats_date) OVER  (PARTITION BY short_code) min_real_reach,
        argMax(real_reach, stats_date) OVER  (PARTITION BY short_code) max_real_reach,
        min_real_reach == 0 real_reach_missing_for_any_date,
        max_real_reach > 0 real_reach_available_for_any_date,
        if (real_reach_missing_for_any_date == 0, real_reach, fake_reach) reach,

        argMin(real_impressions, stats_date) OVER  (PARTITION BY short_code) min_real_impressions,
        argMax(real_impressions, stats_date) OVER  (PARTITION BY short_code) max_real_impressions,
        min_real_impressions == 0 real_impressions_missing_for_any_date,
        max_real_impressions > 0 real_impressions_available_for_any_date,
        if (real_impressions_missing_for_any_date == 0, real_impressions, fake_impressions) impressions,

        if (total_engagement / reach < 10000, total_engagement / reach, 0) engagement_rate,
...
select
    ...
    coalesce(if(ph.handle = '', NULL, ph.handle), post_handle) profile_handle,
    post_handle,
    ifNull(ph.name, profile_handle) profile_name,
