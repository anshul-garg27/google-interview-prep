...
all_collection_profiles as (
    ...
    where platform = 'YOUTUBE' and profile_social_id != '' and enabled = True and pc.source = 'SAAS-AT'
),
all_profiles as (
...
posts as (
    ...
    from dbt.stg_beat_youtube_post where channel_id in all_profiles
),
shortcodes as (
...
channel_ids as (
    select profile_id from collection_posts group by profile_id
),
channel_names as (
...
left join channel_names cn on cn.channel_id = collection_posts.profile_id
