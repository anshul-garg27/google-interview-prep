{{ config(materialized = 'table', tags=["post_ranker_partial"], order_by='handle') }}
with
recent_posts_by_class as (
    select
        ...
        post_rank,
        post_rank_by_class,
        rank() over (partition by handle, post_class order by likes desc, comments desc, publish_time desc) likes_rank_by_class,
        rank() over (partition by handle, post_class order by comments desc, likes desc, publish_time desc) comments_rank_by_class,
        rank() over (partition by handle, post_class order by plays desc, likes desc, comments desc, publish_time desc) plays_rank_by_class,
        rank() over (partition by handle, post_class order by views desc, likes desc, comments desc, publish_time desc) views_rank_by_class
    from {{ref('mart_instagram_recent_post_stats_partial')}}
    where post_rank_by_class <= 12
),
recent_posts as (
    select
        post_shortcode,
        rank() over (partition by handle order by likes desc, comments desc, publish_time desc) likes_rank,
        rank() over (partition by handle order by comments desc, likes desc, publish_time desc) comments_rank
    from {{ref('mart_instagram_recent_post_stats_partial')}}
    where post_rank <= 12
),
all_recent_posts as (
    ...
    order by handle asc, post_class asc, post_rank asc
),
profile_summary as (
    select
        handle,
        ...
        countIf(post_shortcode, post_class='reels' and plays > 0) _recent_reels_with_plays,
        if (_recent_reels_with_plays = 0, 1, _recent_reels_with_plays) recent_reels_with_plays,
        ...
        recent_reels_plays / recent_reels_with_plays avg_reels_plays,
...
profile_summary_with_outliers as (
    select
        ...
        countIf(post_shortcode, post_class='reels' and plays_rank_by_class >= 3 and plays_rank_by_class <= ps.recent_reels - 2 and plays > 0) _reels_non_outliers_with_plays,
        if(_reels_non_outliers_with_plays = 0, 1, _reels_non_outliers_with_plays) reels_non_outliers_with_plays,
        ...
        reels_plays_non_outliers / reels_non_outliers_with_plays avg_reels_plays_non_outliers,
    ...
    left join profile_summary ps on ps.handle = r.handle
    group by handle
),
final_summary as (
    select
    ...
    left join profile_summary_with_outliers  so on so.handle = summary.handle
),
final as (
    select
        ifNull(final_summary.handle, 'MISSING') handle,
        final_summary.*,
    ...
    from final_summary
    left join {{ ref('stg_beat_instagram_account') }} ia on ia.handle = final_summary.handle
