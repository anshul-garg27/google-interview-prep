...
import (
	...
	"init.bulbul.tv/bulbul-backend/saas-gateway/locale/locale"
)
...
func New(c *gin.Context, config config.Config) *Context {
	...
	ctx := &Context{
		Context: c,
		Logger:  zerolog.New(os.Stdout).Level(zerolog.Level(config.LOG_LEVEL)).With().Str(header.RequestID, c.Keys[header.RequestID].(string)).Logger(),
		Config:  config,
		XHeader: make(http.Header),
		Locale:  locale.LocaleEnglish,
	}
	//ctx.Logger = log.Output(zerolog.ConsoleWriter{Out: os.Stderr})
...
func (c *Context) GenerateUserAuthorization() {
	if c.UserClientAccount != nil && c.Client != nil && c.Client.AppType == "ERP" {
		//c.XHeader.Set(header.AccountId, strconv.Itoa(c.UserClientAccount.Id))
		c.ERPUserAuthorization = "Basic " + base64.StdEncoding.EncodeToString([]byte(fmt.Sprintf("%d%s%s%s%d%s%d%s%d", c.UserClientAccount.UserId, "~", getName(*c.UserClientAccount), "~", 1, "~", c.UserClientAccount.Id, ":", c.UserClientAccount.UserId)))
		c.UserAuthorization = c.ERPUserAuthorization
		c.DeviceAuthorization = c.UserAuthorization
	} else if c.UserClientAccount != nil {
		//c.XHeader.Set(header.AccountId, strconv.Itoa(c.UserClientAccount.Id))
		c.UserAuthorization = "Basic " + base64.StdEncoding.EncodeToString([]byte(fmt.Sprintf("%d%s%s%s%d%s%d%s%d", c.UserClientAccount.UserId, "~", getName(*c.UserClientAccount), "~", 1, "~", c.UserClientAccount.Id, ":", c.UserClientAccount.UserId)))
		c.DeviceAuthorization = c.UserAuthorization
		...
		c.UserAuthorization = c.DeviceAuthorization
	}
}
