...
func GetChannel(config config.Config) chan map[string]interface{} {
	channelInit.Do(func() {
		eventWrapperChannel = make(chan map[string]interface{}, config.VIDOOLY_EVENT_WORKER_POOL_CONFIG.EVENT_BUFFERED_CHANNEL_SIZE)
		initWorkerPool(config, config.VIDOOLY_EVENT_WORKER_POOL_CONFIG.EVENT_WORKER_POOL_SIZE, eventWrapperChannel, config.VIDOOLY_EVENT_SINK_CONFIG)
	})
...
func worker(config config.Config, eventChannel <-chan map[string]interface{}) {
	for e := range eventChannel {
		...
		if b, err := json.Marshal(e); err != nil || b == nil || e["name"] == nil {
			...
			err := rabbitConn.Publish(config.VIDOOLY_EVENT_SINK_CONFIG.EVENT_EXCHANGE, strings.ToUpper(e["name"].(string)), b, map[string]interface{}{})
			if err != nil {
